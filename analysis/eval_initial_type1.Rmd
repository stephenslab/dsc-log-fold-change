...
p1 <- make_plots(res_merge, alpha=.001,
                ...
                ggtitle("Type error at alpha < .001, 50/group") + ylim(0,.03) +
                geom_hline(yintercept=.001, col="gray30", lty=1) 
p1
...
#    group_by(method) %>%
    # summarise(mn=mean(type1, na.rm=T), 
    #           n=sum(!is.na(type1)), se=sd(type1, na.rm=T)/sqrt(n)) %>%
    ggplot(., aes(x=method, y=log2(type1), col=method)) +
        # geom_errorbar(aes(ymin=mn+se, ymax=mn-se), width=.3) + 
        geom_boxplot() + geom_point(size=.7) + xlab("") +
      scale_x_discrete(position = "top",
                       labels=args$labels) +
...
# a <- res_merge[res_merge$n1==50 & res_merge$method=="wilcoxon",] %>%
#   group_by(seed) %>%
